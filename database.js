...
const initDatabase = async () => {
    try {
        ...
        await run(`CREATE TABLE IF NOT EXISTS valve_ownership_transfers (
            ...
            from_owner_type VARCHAR(20) NOT NULL CHECK (from_owner_type IN ('manufacturer', 'distributor', 'burned')),
            to_owner_id VARCHAR(50) NOT NULL,
            to_owner_type VARCHAR(20) NOT NULL CHECK (to_owner_type IN ('manufacturer', 'distributor', 'burned')),
            transfer_type VARCHAR(20) NOT NULL CHECK (transfer_type IN ('initial_assignment', 'transfer', 'revoke', 'burn', 'restore')),

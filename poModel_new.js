const { db, generateUUID } = require('./database_new');
...
class PurchaseOrder {
    ...
    // Create a new purchase order with UUID
    static async create(poData) {
        ...
        const sql = `INSERT INTO purchase_orders 
                     (id, po_number, manufacturer_id, distributor_id, total_amount, currency, items, notes, status) 
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending')`;
